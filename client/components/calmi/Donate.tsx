...
export default function Donate({ defaultEmail = "" }: { defaultEmail?: string }) {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        if (!j?.paystackPublicKey) console.warn("Calmi: PAYSTACK_PUBLIC_KEY not configured");
      } catch (e) {
  ...
  function ensurePaystackLoaded(timeoutMs = 8000) {
    return new Promise<void>((resolve, reject) => {
      ...
      if (s) s.addEventListener("load", () => resolve(), { once: true });
      check();
    });
  }
  ...
  const pay = async () => {
    ...
    if (!publicKey) {
      alert("Payments unavailable: server missing PAYSTACK_PUBLIC_KEY.");
      return;
    }
